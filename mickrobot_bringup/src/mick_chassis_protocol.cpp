...
namespace MickRobot
{
	...
	/**
	 * @brief 创建一个新的Mick Robot CHASSIS对象
	 *
	 * @param mode 通信模式 (0表示串口通信, 1表示UDP通信)
	 * @param chassis_type 底盘类型 (0: 差速驱动, 1: 麦克纳姆轮, 2: 阿克曼转向, 3: 4WS4WD)
	 * @param dev 串口设备路径
	 * @param baud 串口通信波特率
	 * @param udp_serv_ip UDP服务器IP地址
	 * @param udp_serv_port UDP服务器端口号
	 */
	MickRobotCHASSIS::MickRobotCHASSIS(int mode, int chassis_type,
									   ...
									   std::string udp_serv_ip, int udp_serv_port)
	{
		...
		if (udp_serv_ip.empty())
		{
			udp_serv_ip_ = "192.168.1.30";
			udp_port_ = 1231;
		}
		else
		{
			udp_serv_ip_ = udp_serv_ip;
			udp_port_ = udp_serv_port;
		}

		if (communication_mode_ == 0)
		{
		...
		{ // UDP_MODE
			...
			// Clean  servaddr_ and cliaddr_
			memset(&servaddr_, 0, sizeof(servaddr_));
			memset(&cliaddr_, 0, sizeof(cliaddr_));

			servaddr_.sin_family = AF_INET; // IPv4
			servaddr_.sin_port = htons(udp_port_);
			servaddr_.sin_addr.s_addr = inet_addr(udp_serv_ip_.c_str());

			// Filling client information (local)
			cliaddr_.sin_family = AF_INET;
			cliaddr_.sin_port = htons(udp_port_);
			cliaddr_.sin_addr.s_addr = INADDR_ANY; // Bind to any available interface

			if (bind(sockfd_, (const struct sockaddr *)&cliaddr_, sizeof(cliaddr_)) < 0)
			{
				perror("bind failed");
				close(sockfd_);
				return;
	...
	MickRobotCHASSIS::~MickRobotCHASSIS()
	{
		if (communication_mode_ == 0 && serial_port_)
		{
			if (serial_port_->is_open())
			{
				serial_port_->close();
			}
			// 依赖shared_ptr自动管理，移除delete语句
			serial_port_.reset();
		}

		// 添加UDP套接字关闭逻辑
		if (communication_mode_ == 1 && sockfd_ >= 0)
		{
			close(sockfd_);
			sockfd_ = -1;
		}
	...
	/**
	 * @brief 从通信接口(串口或UDP)读取数据
	 *
	 * @return true 如果成功读取并处理了数据
	 * @return false 如果读取或处理失败
	 */
	bool MickRobotCHASSIS::readData(void)
	{
		// 读取数据
		std::string tmp_string;
		if (read_bytes_from_udp_or_serial(tmp_string) <= 5)
		{
			printf("recive data len <= 5 ");
			return false;
		}
	 ...
	 * @brief 解析从底盘接收到的数据帧
	 *
	 * @param str_data 从底盘接收到的原始数据字符串
	 * @return true 如果成功解析了数据
	 * @return false 如果解析失败
	 */
		...
		for (int k = 0; k < header_count; k++)
		{
			...
			{ // 检查帧头帧尾是否完整
				...
				else if (reviced_tem[3 + step] == 0xA7) // mickv3 里程计
				{
					...
					odom.int16_dat = 0;
					odom.int16_dat = 0;
					odom.byte_data[1] = reviced_tem[i++];
					odom.byte_data[0] = reviced_tem[i++];
				...
				else if (reviced_tem[3 + step] == 0xA0) // IMU 数据
				{
					...
					imu.int16_dat = 0;
					imu.byte_data[1] = reviced_tem[i++];
					imu.byte_data[0] = reviced_tem[i++];
				...
				else if (reviced_tem[3 + step] == 0xA1)
				{
					...
					for (int j = 0; j < 10; j++)
					{
						ultra_tem = reviced_tem[i++] * 256;
						ultra_tem = ultra_tem + reviced_tem[i++];
	...
	/**
	 * @brief 根据底盘类型发送速度指令到底盘
	 *
	 * @param speed_x X轴线速度 (米/秒)
	 * @param speed_y Y轴线速度 (米/秒)
	 * @param speed_w 角速度 (弧度/秒)
	 */
	void MickRobotCHASSIS::send_speed_to_chassis(float speed_x, float speed_y, float speed_w)
	{

		if (chassis_measurements.chassis_type == 0) // mickX4-v3 差速模式
		{
	 ...
	 */
	void MickRobotCHASSIS::send_rpm_to_chassis(int w1, int w2, int w3, int w4)
	{
		uint8_t data_tem[50];
		...
		send_bytes_to_udp_or_serial(data_tem, counter);
	}

	/**
	 * @brief 发送速度指令到差速驱动底盘(MickX4)
	 *
	 * @param x X轴线速度 (米/秒)
	 * @param y Y轴线速度 (米/秒)
	 * @param w 角速度 (弧度/秒)
	 */
	void MickRobotCHASSIS::send_speed_to_X4chassis(float x, float y, float w)
	{
		uint8_t data_tem[50];
	...
	void MickRobotCHASSIS::clear_odometry_chassis(void)
	{
		uint8_t data_tem[50];
		unsigned char i, counter = 0;
	...
	/**
	 * @brief 从串口或UDP连接读取字节数据
	 *
	 * @param str_data 包含接收到数据的输出字符串
	 * @return size_t 读取的字节数
	 */
	size_t MickRobotCHASSIS::read_bytes_from_udp_or_serial(std::string &str_data)
	{
		size_t bytes_read = 0;
		uint8_t tmp[1024];
		memset(tmp, 0, 1024);
		...
		if (communication_mode_ == 0)
		{
			bytes_read = serial_port_->read_some(boost::asio::buffer(tmp, 1024));
			if (bytes_read <= 0)
			{
		...
		else if (communication_mode_ == 1)
		{ // UDP_MODE

			socklen_t len_addr = sizeof(servaddr_);
			bytes_read = recvfrom(sockfd_, tmp, sizeof(tmp), MSG_DONTWAIT, (struct sockaddr *)&servaddr_, &len_addr);
			if (bytes_read == -1)
			{
			...
			tmp[bytes_read] = '\0'; // Null-terminate the received data
		}

		// 输出读取的数据以进行调试
		printf("\n Serial Read %zu bytes", bytes_read);
		for (size_t i = 0; i < bytes_read; ++i)
		{
			str_data.push_back(tmp[i]);
			// printf("Data[%d]: 0x%02X", i, tmp_string.at(i));
		}
		// printf("\n");
	...
	/**
	 * @brief 通过串口或UDP连接发送字节数据
	 *
	 * @param data 指向待发送数据缓冲区的指针
	 * @param length 待发送数据的长度
	 */
	void MickRobotCHASSIS::send_bytes_to_udp_or_serial(uint8_t *data, size_t length)
	{
		if (length == 0)
		{
			printf("Warning: Attempted to send empty data\n");
			return;
		}
		if (communication_mode_ == 0)
		{
			...
			// 	return;
			// }
			// boost::asio::write(serial_port_, boost::asio::buffer(data, length));
			size_t bytesWritten = serial_port_->write_some(boost::asio::buffer(data, length));
			//  if (bytesWritten != length)
			//  {
			//  	printf("Warning: Only %zu/%zu bytes written to serial port\n",
			//  		   bytesWritten, length);
			//  }
		}
		else if (communication_mode_ == 1) // UDP发送模式
		{
			if (sockfd_ < 0)
			{
			...
			if (sendto(sockfd_, data, length, 0, (const struct sockaddr *)&servaddr_, sizeof(servaddr_)) == -1)
			{
				printf("sendto failed in X4chassis");
			}
		}
	}
